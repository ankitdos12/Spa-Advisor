...
const SpaImageGallery = ({ images }) => {
    ...
    return (
        <div>
            {/* Main large image */}
            <div
                className="h-[500px] overflow-hidden rounded-lg mb-4 cursor-pointer"
            ...
            <div className="grid grid-flow-col auto-cols-max gap-2 overflow-x-auto pb-2">
                {images.map((image, index) => (
                    <div
                        key={index}
                        className={`h-20 w-20 overflow-hidden rounded cursor-pointer ${activeIndex === index ? 'ring-2 ring-blue-500' : ''
                            }`}
